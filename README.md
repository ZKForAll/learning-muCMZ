...
- **Algebraic MAC (§3.2, Def 3.1).** `MAC = (S, K, M, V)`:
  - `crs ← S(1^λ, n)` — setup, fixes attribute count n and message family 𝕄.
  - `(sk, pp) ← K(crs)` — keygen.
  - `σ ← M(sk, m⃗)` — MAC over attributes `m⃗ ∈ 𝕄ⁿ` (randomized).
  - `0/1 := V(sk, m⃗, σ)` — deterministic verification.
...
generator `𝒢 = 4` (order 11 in (ℤ₂₃)ˣ). Tiny, so outputs are readable — DL is of
course trivially breakable at this size; bump `p, q` to crypto sizes to make it
hard. The *algebra* (homomorphisms, binding) is honest at any size.
...
- [ ] **0a** Prime-order group — `PrimeOrderGroup` interface + Schnorr instance
